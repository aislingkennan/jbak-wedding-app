...
export default function AfterPartyForm() {
  ...
  const [shuttle, setShuttle] = useState<boolean | null>(null);
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    if (!name.trim() || attending === null || shuttle === null) return;
    setLoading(true);
    setError('');
    try {
      const res = await fetch('/api/after-party', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ name: name.trim(), attending, partnerName: partnerName.trim(), partnerAttending, shuttle }),
      });
  ...
  return (
    <form onSubmit={handleSubmit} className="p-6">
        ...
        <div>
          ...
          <div className="flex gap-3">
            <button
              ...
              className="flex-1 py-2 px-3 text-xs tracking-wider border transition-colors"
              style={{
                backgroundColor: shuttle === true ? '#C9A84C' : 'transparent',
                color: shuttle === true ? '#fff' : '#6b5e4e',
                borderColor: shuttle === true ? '#C9A84C' : '#d4c4a8',
              }}
            >
              Yes please
            </button>
            <button
              ...
              className="flex-1 py-2 px-3 text-xs tracking-wider border transition-colors"
              style={{
                backgroundColor: shuttle === false ? '#8a7a6a' : 'transparent',
                color: shuttle === false ? '#fff' : '#6b5e4e',
                borderColor: shuttle === false ? '#8a7a6a' : '#d4c4a8',
              }}
            >
              No thanks
            </button>
      ...
      <button
        type="submit"
        disabled={!name.trim() || attending === null || shuttle === null || loading}
